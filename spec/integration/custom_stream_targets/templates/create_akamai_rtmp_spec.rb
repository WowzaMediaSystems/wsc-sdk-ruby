...
describe "custom_stream_target .save akamai_rtmp", :integration_test => true do
  
  name                    = "auto sdk save akamai_rtmp"
  primary_url             = "rtmp://10.0.0.1/live"
  stream_name             = "auto_stream_name"
  location                = "us_west_california"
  provider                = "akamai_rtmp"
  id                      = /\w{8}/

  before :all do
    @custom_stream_targets      = $client.stream_targets.custom
    data = WscSdk::Templates::CustomStreamTarget.akamai_rtmp(name, primary_url, stream_name)
